    ...
    switch (uMsg) {
        case WM_CREATE:{ // ウィンドウが作られた瞬間
            ...
            for (int i = 0; i < 16; i++) {
                ...
                if (wcschr(L"FromTo=C", labels[i][0])) {               // 演算子（+, -, *, /, =, C）なら「自分で描く(OWNERDRAW)」スタイルにする
                    style |= BS_OWNERDRAW;
                } else {
                    style |= BS_PUSHBUTTON;
                }
        ...
        case WM_COMMAND: {
            ...
            if (wmId >= 101 && wmId <= 116) {   // ボタン（101～116）が押された場合
                ...
                if (wcscmp(selectedLabel, L"C") == 0) {         // 【Cボタン】 計算過程をすべてリセット
                ...
                else if (wcscmp(selectedLabel, L"EX") == 0) {    // 【＝ボタン】
                    //ーーー通信を行う部分ーーー
                    ...
                    double displayNum = _wtof(currentText);
                    //ーーー計算するーーー
                    double baseInvRate = j["jpy"]["inverseRate"];
                    double quoteRate = j["eur"]["rate"];
                    double result = displayNum * baseInvRate * quoteRate;
                    swprintf(currentText, 64, L"%f", result);
                    SetWindowTextW(hEdit, currentText);
                    isNewInput = true;
                }
                else if (wcscmp(selectedLabel, L"JPY") == 0) {   //　【BASEボタン】
                    //ーーー未定ーーー
                }
                else if (wcscmp(selectedLabel, L"EUR") == 0) {   //　【BASEボタン】
                    //ーーー未定ーーー
                }
                else if (wcscmp(selectedLabel, L"sty") == 0) {   //　【BASEボタン】
                    //ーーー未定ーーー
                }
